...
{
  /*
    Rampart dnscrypt-proxy (dnsmasq variant) Hardening Module

    This module hardens the dnscrypt-proxy instance that forwards DNS queries
    for dnsmasq via DoH/DoT/DNSCrypt. The service is essentially stateless —
    it listens on localhost, encrypts DNS queries, and forwards them to
    upstream resolvers. This allows aggressive sandboxing.

    The base service definition (ExecStart, Restart, etc.) lives in
    modules/networking/dns/dnsmasq.nix. This module adds comprehensive
    systemd sandboxing on top.
  */

  systemd.services.dnscrypt-proxy-dnsmasq.serviceConfig = {
